<?php
  ...
  function linkGroups($userId, $selectedGroepen)
  {
      ...
      $sql = "DELETE FROM linkgroups WHERE userid = :userid AND archive = 0";
      $sth = $conn->prepare($sql);
      $sth->bindParam(':userid', $userId);
      $sth->execute();
  ...
  function logUserUpdate($userId)
  {
      ...
      $sql = "INSERT INTO logs (userid, useragent, action, tableid, interactionid, error) VALUES (:userid, :useragent, 2, 6, :interactionid, 0)";
      $sth = $conn->prepare($sql);
